...
def main():
    ...
    # plot 2 (Position error + RMSE)
    fig2, ax2 = plt.subplots(figsize=(10.5, 4.8))

    # summary numbers for legend
    err_mean = float(np.mean(err))
    err_max = float(np.max(err))

    # Position error: purple solid
    ax2.plot(
        truth["t"], err,
        color="tab:purple", linestyle="-", linewidth=2.0,
        label=f"Position error: mean={err_mean:.2f} m, max={err_max:.2f} m"
    )

    # RMSE: black dashed horizontal line
    ax2.axhline(
        rmse_val,
