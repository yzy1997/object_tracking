...
from src.CA_EKF               import KalmanPredictor   # 连续加速度 EKF
...
def main():
    ...
    predictor = KalmanPredictor(process_var=0.5, meas_var=10.0)

    actual_centers    = []
    predicted_centers = []
    Vmax = 20.0  # 最大像素速度约束 px/frame
    ...
    for idx, fp in enumerate(frame_files, start=1):
        ...
        if actual_centers[-1] is not None and predicted_centers[-1] is not None:
            ...
            ax.plot([lt[0], nt[0]], [lt[1], nt[1]],
                    'r--', lw=1.5)
            ax.plot(nt[0], nt[1], 'rx', mew=2, markersize=8)
    ...
    preds_xs = np.vstack([p for p in predicted_centers[:-1] if p is not None])

    plt.plot(pts_all[:,0], pts_all[:,1], '-o', color='white',
             markersize=4, linewidth=1.2, label='True Track')
    plt.plot(preds_xs[:,0], preds_xs[:,1], 'rx',
             markersize=6, label='Pred Points')
    ...
    plt.axis('off')
    plt.savefig(os.path.join(out_folder, "summary.png"),
                bbox_inches='tight', pad_inches=0)
    plt.close()
    ...
    plt.plot(pts_all[:,0], pts_all[:,1], '-o', color='white',
             markersize=4, linewidth=1.2, label='True Track')
    # 预测轨迹折线（按顺序连接预测点）
    plt.plot(preds_xs[:,0], preds_xs[:,1], '--o', color='red',
             markersize=4, linewidth=1.2, label='Predicted Track')
