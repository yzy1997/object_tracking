...
- gt_tracks.csv          : true GT for evaluation
- summary_hits.png
- summary_max.png
"""
...
GT_CSV = os.path.join(OUT_DIR, "gt_tracks.csv")
...
def save_gt_csv(gt_rows, path: str):
    os.makedirs(os.path.dirname(path), exist_ok=True)
...
def main():
    ...
    for t in range(maxT):
        ...
        do_paint = (t % SKIP) == 0
        if do_paint:
            for gid, u in enumerate(uavs, start=1):
                yx = u.step()
                if yx is None:
                    continue
    ...
    for i, u in enumerate(uavs):
        ...
        print(f"  UAV{i+1}: dir={direction}, |vx|={abs(u.vx):.2f} px/frame, max_steps={u.max_steps}")
